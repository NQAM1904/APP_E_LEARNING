...
export default class LichHocComponent extends Component {
  ...
  getMonHoc = (day) => {
    const result = fetch(
      `${API_URL}/GetMonHocByDay?IDSINHVIEN=${userData.IDSTUDENT}&Day=${day}`,
      {
        method: 'GET',
        headers: {
  ...
  async componentDidMount() {
    this.props.navigation.addListener('focus', async () => {
      ...
      const result = await fetch(
        `${API_URL}/GetTKB?IDSINHVIEN=${userData.IDSTUDENT}`,
        {
          method: 'GET',
          headers: {
            Accept: 'application/json',
      ...
      if (result != undefined) {
        if (result.Success == true) {
          ...
          result.Data.map((item) => {
            ...
            DataStart.setFullYear(
              parseInt(item.THOIGIANBATDAU.substring(0, 4)),
              parseInt(item.THOIGIANBATDAU.substring(5, 7)) - 1,
              parseInt(item.THOIGIANBATDAU.substring(8, 10)),
            );
            DataEnd.setFullYear(
              parseInt(item.THOIGIANKETTHUC.substring(0, 4)),
              parseInt(item.THOIGIANKETTHUC.substring(5, 7)) - 1,
              parseInt(item.THOIGIANKETTHUC.substring(8, 10)),
            );
            ...
            while (DataStart <= DataEnd) {
              if (
                DataStart.getDay() == dateNeed[0] ||
                DataStart.getDay() == dateNeed[1] ||
                DataStart.getDay() == dateNeed[2]
              ) {
          ...
          this.setState({
            DataTickArr: ArrDataMark,
            DataTick: this.toObject(ArrDataMark),
          });
        } else {
          // console.log(result)
        ...
        result = selectItem.map((item, index) => {
          return (
            <ItemLH
              ...
              key={index}
              CAHOC={
                item.LICHOC.CAHOC1 || item.LICHOC.CAHOC2 || item.LICHOC.CAHOC3
              }
      ...
      <View style={styles.container}>
        ...
        <View style={{ height: 60, width: '100%' }}>
          <View
            ...
            <TouchableOpacity
              onPress={() => {
                this.props.navigation.navigate('DangKyMH');
              }}>
              <Image
                source={images.plus}
